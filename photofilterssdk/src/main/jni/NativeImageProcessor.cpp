...
static inline jint *getPointerArray(JNIEnv *env, jintArray buff) {
    jint *ptrBuff = NULL;
    if (buff != NULL)
        ptrBuff = env->GetIntArrayElements(buff, (unsigned char *) false);
    return ptrBuff;
}
...
Java_com_zomato_photofilters_imageprocessors_NativeImageProcessor_applyRGBCurve(JNIEnv *env,
    ...
    jint *pixelsBuff = getPointerArray(env, pixels_);
    jint *RGBBuff = getPointerArray(env, rgb_);
    applyRGBCurve(width, height, pixelsBuff, RGBBuff);
    jintArray result = jintToJintArray(env, width * height, pixelsBuff);
    releaseArray(env, pixels_, pixelsBuff);
    releaseArray(env, rgb_, RGBBuff);
...
Java_com_zomato_photofilters_imageprocessors_NativeImageProcessor_applyChannelCurves(JNIEnv *env,
                                                                                     ...
                                                                                     jint height) {
    jint *pixels = env->GetIntArrayElements(pixels_, NULL);
    jint *r = env->GetIntArrayElements(r_, NULL);
    jint *g = env->GetIntArrayElements(g_, NULL);
    jint *b = env->GetIntArrayElements(b_, NULL);

    jint *pixelsBuff = getPointerArray(env, pixels_);
    jint *RBuff = getPointerArray(env, r_);
    jint *GBuff = getPointerArray(env, g_);
    jint *BBuff = getPointerArray(env, b_);
    applyChannelCurves(width, height, pixelsBuff, RBuff, GBuff, BBuff);
    jintArray result = jintToJintArray(env, width * height, pixelsBuff);
    releaseArray(env, pixels_, pixelsBuff);
    releaseArray(env, r_, RBuff);
    releaseArray(env, g_, GBuff);
    releaseArray(env, b_, BBuff);

    env->ReleaseIntArrayElements(pixels_, pixels, 0);
    env->ReleaseIntArrayElements(r_, r, 0);
    env->ReleaseIntArrayElements(g_, g, 0);
